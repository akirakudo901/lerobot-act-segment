...
"""Policy-agnostic hybrid chunk queue / MP executor for segment IL policies."""
...
class IkPending:
    ...
    :meth:`SegmentRolloutWrapper.set_rollout_action_processors` has been configured
...
def first_mp_frame_index(labels_row: Tensor) -> int | None:
...
def first_contiguous_mp_run(labels_row: Tensor) -> tuple[int, int] | None:
...
def effective_chunk_horizon(
    labels_row: Tensor,
    *,
    n_action_steps: int,
    hybrid_refill_mode: str,
    hybrid_refill_mp_trust_steps: int = 5,
) -> int:
    """Truncate a predicted label row to the hybrid refill horizon."""
    mode = hybrid_refill_mode
    if mode == "full_chunk":
        return n_action_steps

    if mode in ("until_first_mp_segment", "trust_contiguous_mp"):
        run = first_contiguous_mp_run(labels_row)
        if run is None:
            return n_action_steps
        start, end = run
    ...
    first_mp = first_mp_frame_index(labels_row)
    if first_mp is None:
        return n_action_steps
    ...
    if mode == "trust_near_mp":
        k = hybrid_refill_mp_trust_steps
        if first_mp < k:
            return first_mp + 1
        return first_mp
...
class SegmentRolloutWrapper:
    """Hybrid chunk queue + MP executor over any :class:`SegmentChunkPolicy`.

    Owns refill modes (including contiguous-MP-run horizons), connector wiring,
    IK / OMPL execution loops, and eval-time action postprocessing / MP rescaling.
    """

    def __init__(
        self,
        policy: SegmentChunkPolicy,
        config: Any,
        *,
        ...
        if getattr(config, "hybrid_refill_mode", None) == "trust_near_mp" and int(
            config.hybrid_refill_mp_trust_steps
        ) < 0:
            raise ValueError(
        ...
        if bool(getattr(config, "use_hybrid_orchestrator", False)):
            self._connector = self._make_hybrid_connector()

        self._rollout_postprocessor: Any | None = None
        self._mp_rescaling_ctx: MpActionRescalingRolloutContext | None = (
            self._init_mp_rescaling_context(
    ...
    def _init_mp_rescaling_context(
        ...
        *,
        ...
        path = Path(pretrained_path) if pretrained_path else None
        if path is None:
            cfg_path = getattr(self.config, "pretrained_path", None)
    ...
    def reset(self) -> None:
        """Clear hybrid orchestrator chunk state."""
        self._select_action_batchsize: int | None = None
    ...
    def bind_eval_env(self, env: Any | None) -> None:
        """Associate this wrapper with a VectorEnv for hybrid MP (Layer-1 OMPL RPC).

        For ``mp_executor_type='ompl_waypoints'``, ``select_action`` calls
    ...
    def _mp_rescaling_keys_for_batch(
        ...
        Require ``mp_rescale_key`` (e.g. for LIBERO, task stem from eval).
        """
    ...
    def _first_mp_frame_index(self, labels_row: Tensor) -> int | None:
        return first_mp_frame_index(labels_row)

    def _first_contiguous_mp_run(self, labels_row: Tensor) -> tuple[int, int] | None:
        return first_contiguous_mp_run(labels_row)

    def _effective_chunk_horizon(self, labels_row: Tensor) -> int:
        return effective_chunk_horizon(
            labels_row,
            n_action_steps=int(self.config.n_action_steps),
            hybrid_refill_mode=str(self.config.hybrid_refill_mode),
            hybrid_refill_mp_trust_steps=int(self.config.hybrid_refill_mp_trust_steps),
        )
    ...
    def _refill_hybrid_rows(
        ...
        *,
        ...
        """Recompute action/label chunk rows for the hybrid orchestrator."""
        ...
        actions, labels = self.policy.predict_action_label_chunk(
    ...
    def _plan_ompl_rows_with_retries(
        ...
        while pending:
            ...
            if requery_rows:
                # ACT-only knob when present; other backbones decode deterministically.
                sample_latent = bool(getattr(self.config, "ompl_retry_sample_latent", False))
                self._refill_hybrid_rows(
                    batch,
                    requery_rows,
                    sample_latent_prior=sample_latent,
                )
            pending = next_pending

    @torch.no_grad()
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        """Select one action per env; route MP triggers to IK dummy or OMPL OSC."""
