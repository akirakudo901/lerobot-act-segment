...
"""Shared hybrid-rollout + segment-label CE config fields for any segment policy."""
...
@dataclass
class SegmentPolicyConfigMixin:
    """Mixin for hybrid inference + MP/L-separated label CE knobs.

    Combine with a backbone ``PreTrainedConfig`` (e.g. ``ACTConfig``) via multiple
    inheritance. Expects the host config to also provide ``n_action_steps`` /
    ``chunk_size`` for refill horizon logic.
